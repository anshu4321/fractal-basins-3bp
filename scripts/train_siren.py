"""Train the SIREN baseline classifier and compare against the MLP baseline.

SIREN (sinusoidal activation network) is the standard remedy for MLP spectral
bias on high-frequency / fractal targets. The 3BP basin partition has sharp
boundaries that the plain MLP baseline smooths out (see baseline_errors.png
from chunk 3 — errors concentrate exactly on those boundaries).

This script mirrors train_baseline.py but:
  - Uses mega3bp.siren.BasinSiren with omega_0 = 30
  - Raw (n_1, n_2, n_3) input — SIREN handles high frequencies natively so
    Fourier features are redundant
  - Trains for 60 epochs, best-val-acc checkpoint selection
  - At the end, loads data/baseline_history.npz (the MLP results) and emits
    a comparison figure showing SIREN vs MLP on the same test split

Outputs:
    data/siren_model.eqx
    data/siren_history.npz
    figures/siren_curves.png
    figures/siren_confusion.png
    figures/siren_mollweide.png
    figures/siren_errors.png
    figures/siren_summary.png
    figures/siren_vs_mlp.png          side-by-side comparison
"""
...
from mega3bp.ml import N_CLASSES, loss_fn, predict, stratified_split
from mega3bp.siren import BasinSiren
...
def main() -> int:
    ...
    parser.add_argument("--lr", type=float, default=1e-4)  # SIREN prefers lower LR
    ...
    print(f"SIREN omega_0: {args.omega_0}")
    print()
    ...
    print(f"kept {len(df):,} non-ambiguous rows")
    ...
    x_train = jnp.asarray(raw[train_idx])
    ...
    opt = optax.adamw(learning_rate=args.lr, weight_decay=1e-5)
    ...
    for epoch in range(args.epochs):
        ...
        print(
            ...
            f"val_mse {float(val_metrics['mse_loss']):.4f}  [{dt:.2f}s]{star}"
        )
    ...
    gate_r2 = r2 >= 0.80
    print(f"  contract gates:")
    print(f"    acc >= 0.90 : {'PASS' if gate_acc else 'FAIL'}  ({overall_acc:.4f})")
    ...
    eqx.tree_serialise_leaves(str(out_data / "siren_model.eqx"), model)
    np.savez(
        out_data / "siren_history.npz",
    ...
    print(f"saved weights and history")
    print()
    ...
    axes[1].axhline(trivial_acc, color=PALETTE["coral"], ls="--", lw=1.1, label=f"trivial = {trivial_acc:.3f}")
    axes[1].axhline(0.90, color=PALETTE["amber"], ls=":", lw=1.0, label="contract gate = 0.90")
    ...
    axes[2].set_title("val masked log-escape MSE")
    ...
    fig.suptitle(
        f"SIREN training  ·  hidden={args.hidden}×{args.layers}  ·  omega_0={args.omega_0}  ·  lr={args.lr}",
        color=PALETTE["text"], fontsize=14, y=1.02,
    )
    fig.tight_layout()
    fig.savefig(out_fig / "siren_curves.png", facecolor=PALETTE["bg_deep"])
    print(f"  saved {out_fig / 'siren_curves.png'}")
    plt.close(fig)
    ...
    ax.set_ylabel("true")
    ax.set_title(
        f"SIREN confusion matrix  ·  macro F1 = {macro_f1:.3f}",
        color=PALETTE["text"],
    )
    ...
    fig.tight_layout()
    fig.savefig(out_fig / "siren_confusion.png", facecolor=PALETTE["bg_deep"])
    print(f"  saved {out_fig / 'siren_confusion.png'}")
    plt.close(fig)

    # 3. Mollweide predicted vs true
    n_test = raw[test_idx]
    ...
    mollweide_basin(axes[1], y_pred, "SIREN prediction")
    fig.suptitle(
        f"SIREN · predicted vs true basin on {len(y_true):,} held-out ICs",
        color=PALETTE["text"], fontsize=14, y=1.02,
    )
    fig.tight_layout()
    fig.savefig(out_fig / "siren_mollweide.png", facecolor=PALETTE["bg_deep"])
    print(f"  saved {out_fig / 'siren_mollweide.png'}")
    plt.close(fig)
    ...
    ax.set_title(
        f"SIREN errors  ·  {int(wrong.sum()):,} / {len(y_true):,} = {100*wrong.mean():.2f}%",
    ...
    for t in leg.get_texts():
        t.set_color(PALETTE["text"])
    fig.tight_layout()
    fig.savefig(out_fig / "siren_errors.png", facecolor=PALETTE["bg_deep"])
    print(f"  saved {out_fig / 'siren_errors.png'}")
    plt.close(fig)
    ...
    mollweide_basin(ax1, y_pred, "SIREN predicted basin")
    ...
    ax2.invert_yaxis()
    ax2.set_xlabel("F1")
    ax2.set_title("SIREN per-class F1", color=PALETTE["text"])
    ...
    ax3.set_title("SIREN accuracy over training")
    ...
    ax4.imshow(norm, cmap="magma", vmin=0, vmax=1.0, aspect="auto")
    for i in range(N_CLASSES):
        for j in range(N_CLASSES):
            col = PALETTE["text"] if norm[i, j] < 0.5 else PALETTE["bg_deep"]
    ...
    lines.extend([
        ...
        "",
        f"arch               : SIREN [{args.hidden}x{args.layers}]",
        f"omega_0            : {args.omega_0}",
    ...
    fig.suptitle("SIREN baseline on 1M-IC basin dataset",
                 color=PALETTE["text"], fontsize=16, y=1.005)
    fig.savefig(out_fig / "siren_summary.png", facecolor=PALETTE["bg_deep"])
    print(f"  saved {out_fig / 'siren_summary.png'}")
    plt.close(fig)

    # 6. SIREN vs MLP comparison
    mlp_hist_path = PROJECT_ROOT / "data" / "baseline_history.npz"
    if mlp_hist_path.exists():
        print(f"  loading {mlp_hist_path} for comparison")
        mlp = np.load(mlp_hist_path, allow_pickle=True)

        fig = plt.figure(figsize=(16, 10), dpi=150, constrained_layout=True)
        fig.patch.set_facecolor(PALETTE["bg_deep"])
        gs = fig.add_gridspec(2, 3)

        # Row 1: side-by-side Mollweide predictions
        axA = fig.add_subplot(gs[0, 0], projection="mollweide")
        axB = fig.add_subplot(gs[0, 1], projection="mollweide")
        axC = fig.add_subplot(gs[0, 2], projection="mollweide")

        mlp_shape = mlp["shape_test"]
        mlp_lon = np.arctan2(mlp_shape[:, 1], mlp_shape[:, 0])
        mlp_lat = np.arcsin(np.clip(mlp_shape[:, 2], -1, 1))

        def moll_draw(ax, lab_arr, lon_arr, lat_arr, title):
            ax.set_facecolor(PALETTE["bg_deep"])
            ax.grid(color=PALETTE["grid"], alpha=0.4, lw=0.3)
            ax.tick_params(colors=PALETTE["text_mute"], labelsize=6)
            size = max(0.4, 9000 / len(lab_arr))
            bound = lab_arr == 0
            ax.scatter(lon_arr[bound], lat_arr[bound], s=size,
                       c=BASIN_COLORS["bound"], alpha=0.25,
                       edgecolors="none", rasterized=True)
            for i in range(1, 4):
                m = lab_arr == i
                if m.any():
                    ax.scatter(lon_arr[m], lat_arr[m], s=size * 3.0,
                               c=BASIN_COLORS[CLASS_NAMES[i]], alpha=0.9,
                               edgecolors="none", rasterized=True)
            ax.set_title(title, color=PALETTE["text"])

        moll_draw(axA, np.asarray(mlp["y_test"]), mlp_lon, mlp_lat, "ground truth")
        moll_draw(axB, np.asarray(mlp["y_pred"]), mlp_lon, mlp_lat,
                  f"MLP baseline (acc {float(mlp['overall_acc']):.3f})")
        moll_draw(axC, y_pred, lon, lat,
                  f"SIREN (acc {overall_acc:.3f})")

        # Row 2: comparison bar charts and metric table
        axD = fig.add_subplot(gs[1, 0])
        axD.set_facecolor(PALETTE["bg_panel"])
        mlp_f1 = np.asarray(mlp["per_class_f1"])
        x_pos = np.arange(N_CLASSES)
        width = 0.38
        axD.bar(x_pos - width/2, mlp_f1, width, color=PALETTE["coral"], alpha=0.9,
                label="MLP", edgecolor=PALETTE["bg_deep"], linewidth=0.4)
        axD.bar(x_pos + width/2, f1_vals, width, color=PALETTE["cyan"], alpha=0.9,
                label="SIREN", edgecolor=PALETTE["bg_deep"], linewidth=0.4)
        axD.set_xticks(x_pos, labels=CLASS_NAMES, rotation=20, color=PALETTE["text"], fontsize=9)
        axD.set_ylabel("F1")
        axD.set_title("per-class F1 comparison", color=PALETTE["text"])
        axD.set_ylim(0, 1)
        leg = axD.legend()
        leg.get_frame().set_facecolor(PALETTE["bg_elevate"])
        leg.get_frame().set_edgecolor(PALETTE["spine"])
        ...
        axE.plot(np.arange(1, len(mlp["val_acc"]) + 1), mlp["val_acc"],
                 color=PALETTE["coral"], lw=1.8, label="MLP val")
        axE.plot(epochs_axis, history["val_acc"],
                 color=PALETTE["cyan"], lw=1.8, label="SIREN val")
        axE.axhline(trivial_acc, color=PALETTE["text_mute"], ls="--", lw=0.8, label="trivial")
        axE.axhline(0.90, color=PALETTE["amber"], ls=":", lw=0.9, label="gate")
        axE.set_xlabel("epoch")
        axE.set_ylabel("val accuracy")
        axE.set_ylim(0.85, 1.0)
        axE.set_title("val accuracy over training")
        leg = axE.legend(fontsize=8)
        leg.get_frame().set_facecolor(PALETTE["bg_elevate"])
        leg.get_frame().set_edgecolor(PALETTE["spine"])
        for t in leg.get_texts():
            t.set_color(PALETTE["text"])
        ...
        cmp_lines = [
            r"$\bf{MLP\ vs\ SIREN}$", "",
            f"{'metric':<22}{'MLP':>9}{'SIREN':>9}{'Δ':>9}",
            f"{'-' * 49}",
            f"{'accuracy':<22}{float(mlp['overall_acc']):>9.4f}{overall_acc:>9.4f}{overall_acc - float(mlp['overall_acc']):>+9.4f}",
            f"{'macro F1':<22}{float(mlp['macro_f1']):>9.4f}{macro_f1:>9.4f}{macro_f1 - float(mlp['macro_f1']):>+9.4f}",
            f"{'log-escape R^2':<22}{float(mlp['log_escape_r2']):>9.4f}{r2:>9.4f}{r2 - float(mlp['log_escape_r2']):>+9.4f}",
            f"{'trivial baseline':<22}{trivial_acc:>9.4f}",
            "",
            "per-class F1:",
        ]
        for i, name in enumerate(CLASS_NAMES):
            cmp_lines.append(
                f"  {name:<14}{float(mlp_f1[i]):>8.3f}{f1_vals[i]:>8.3f}{f1_vals[i] - float(mlp_f1[i]):>+8.3f}"
            )
        for i, line in enumerate(cmp_lines):
            axF.text(0.02, 0.97 - i * 0.05, line, transform=axF.transAxes,
                     ha="left", va="top", fontsize=9,
                     color=PALETTE["text"], family="monospace")

        fig.suptitle(
            "Phase 2 chunk 4 — MLP baseline vs SIREN on 1M-IC basin dataset",
            color=PALETTE["text"], fontsize=16, y=1.005,
        )
        fig.savefig(out_fig / "siren_vs_mlp.png", facecolor=PALETTE["bg_deep"])
        print(f"  saved {out_fig / 'siren_vs_mlp.png'}")
        plt.close(fig)
    else:
        print(f"  skipping comparison (no {mlp_hist_path})")
